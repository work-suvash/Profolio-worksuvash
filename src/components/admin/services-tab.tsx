...
const EMPTY: Partial<Service> = { icon_name: 'Code', title: '', description: '', thumbnail: '', link: '', sort_order: 0 };
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">Manage services shown on your portfolio</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {services.length === 0 && (
            <div className="col-span-full text-center py-20 text-muted-foreground bg-card rounded-2xl border border-border/30">
              ...
              <p className="text-sm mt-1 opacity-70">Click "Add Service" to create one</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setEditing(null)}>
          <div className="bg-card border border-border/50 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-foreground">{editing.id ? 'Edit Service' : 'Add Service'}</h2>
