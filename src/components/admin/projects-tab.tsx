...
const CATEGORIES = ['Web Development', 'App Development', 'Newly Added'];
const EMPTY: Partial<AdminProject> = { title: '', description: '', category: 'Web Development', type: 'image', thumbnail: '', src: '', live_link: '', views: 0, likes: 0 };
...
export default function ProjectsTab() {
  ...
  async function handleSave() {
    ...
    if (!error) {
      toast({ title: editing.id ? 'Project updated!' : 'Project added!' });
      setEditing(null);
      loadProjects();
    } else {
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">Add, edit and manage portfolio projects</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-muted-foreground bg-card rounded-2xl border border-border/30">
          ...
          <p className="text-sm mt-1 opacity-70">Click "Add Project" to create your first one</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setEditing(null)}>
          <div className="bg-card border border-border/50 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-foreground">{editing.id ? 'Edit Project' : 'Add Project'}</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={editing.category ?? 'Web Development'} onChange={e => set('category', e.target.value as AdminProject['category'])}
                    className="w-full px-3 py-2 bg-background border border-border/50 rounded-lg text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary/50">
              ...
              <div>
                <label className="text-sm font-medium text-foreground mb-1 block">Live Preview Link (optional)</label>
